...
func TestParseParamStructCodeExample(t *testing.T) {
	...
	import structs "github.com/rampnow-io/swag/testdata/param_structs"
	`, goparser.ParseComments)
	assert.NoError(t, err)

	parser := New()
	err = parser.parseFile("github.com/rampnow-io/swag/testdata/param_structs", "testdata/param_structs/structs.go", nil, ParseModels)
	assert.NoError(t, err)
